...
const seedAll = async () => {
  try {
    ...
    const folder = path.dirname(fallbackDbPath);
    ...
    const demoUser = {
      ...
      activity: [
        { date: new Date(Date.now() - 1000 * 60 * 60 * 48).toISOString(), action: 'Joined Hub', details: 'Welcome to CampusPrep Hub!' },
        { date: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(), action: 'Completed Profile Setup', details: 'Configured academic targets.' }
      ]
    };

    const offlineData = {
      users: [demoUser],
      questions: SEED_QUESTIONS.map((q, idx) => ({ _id: `q-${idx}`, id: `q-${idx}`, ...q })),
      tests: SEED_TESTS.map((t, idx) => ({ _id: `t-${idx}`, id: `t-${idx}`, ...t })),
      companies: SEED_COMPANIES.map((c, idx) => ({ _id: `c-${idx}`, id: `c-${idx}`, ...c })),
      jobs: SEED_JOBS.map((j, idx) => ({ _id: `j-${idx}`, id: `j-${idx}`, ...j }))
    };

    fs.writeFileSync(fallbackDbPath, JSON.stringify(offlineData, null, 2));
    console.log('Fallback local JSON DB seeded successfully.');
    ...
    try {
      ...
      console.log('Skipping MongoDB seeding since it is not active. Fallback db.json handles data storage.');
    }
